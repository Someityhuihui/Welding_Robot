...
float CornerClassifier::computeCornerScoreFromVisibleCameras(const CornerPoint& corner,
    ...
    // 只要有可见的相机（≥1），就判定为内凹角（焊缝）
    bool is_concave = (visible_cameras >= 1);
    
    std::cout << "    [可见相机数] " << visible_cameras << " / " << camera_poses.size() << std::endl;
    
    if (is_concave) {
        std::cout << "    [判定] 内凹角（焊缝）" << std::endl;
        return 1.0f;   // 正分表示内凹角
    } else {
        std::cout << "    [判定] 外凸角（边角）" << std::endl;
        return -1.0f;  // 负分表示外凸角
    }
}
